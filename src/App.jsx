  ...
  return (
    <div className="fixed bottom-5 right-5 z-50 space-y-2 max-w-sm pointer-events-none">
      {globalToasts.map(toast => (
        <div
          key={toast.id}
          className={`pointer-events-auto p-4 rounded-xl shadow-2xl border text-xs font-bold flex items-start space-x-3 transition-all transform animate-in slide-in-from-bottom-5 ${
        ...
        >
          ...
          {toast.type === 'info' && <Info className="w-5 h-5 text-sap-blue shrink-0 mt-0.5" />}
